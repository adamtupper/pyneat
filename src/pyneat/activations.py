...
def elu_activation(z):
    return z if z > 0.0 else mp.exp(z) - 1
...
def selu_activation(z):
    ...
    return lam * z if z > 0.0 else lam * alpha * (mp.exp(z) - 1)


def softplus_activation(z):
    z = max(-60.0, min(60.0, 5.0 * z))
    return 0.2 * math.log(1 + mp.exp(z))
...
class ActivationFunctionSet(object):
    """Contains the list of current valid activation functions, including
    methods for adding and getting them.
    """

    def __init__(self):
        self.functions = {}
        self.add('sigmoid', sigmoid_activation)
        self.add('steep_sigmoid', steep_sigmoid_activation)
